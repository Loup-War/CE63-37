...
public class Serment : MonoBehaviour
{
    ...
    public void LazadioRelation()
    {
        ...
        if (player.GetComponent<TacticsMove>().guildConfirm == "Serment")
        {
    ...
    public void SharingSerment()
    {
        pv.RPC("Sermentcost", RpcTarget.AllBuffered, lazadiocost, shoppicacost, nefixiacost, churchcost);
    }

    [PunRPC]
    void Sermentcost(int lazadicos, int shoppicos, int nefixicos,int churchcos)
    {
    ...
    {
        if (player.GetComponent<TacticsMove>().guildConfirm == "Serment" && mainEvent.GetComponent<MainEventSys>().countTurn2 != 0)
        {
            if (citynumber == 1)
            {
    ...
    public void Spcialceti()
    {
        if (player != null)
        {
            if (player.GetComponent<PlayMoment>().guildConfirm == "Serment")
            {
                if (player.GetComponent<PlayMoment>().certificate[0] != 0)
                {
