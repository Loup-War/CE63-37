...
public class Loup : MonoBehaviour
{
    ...
    public void LazadioRelation()
    {
        ...
        if (player.GetComponent<TacticsMove>().guildConfirm == "Loup")
        {
    ...
    public void SharingLoup()
    {
        pv.RPC("Loupcost", RpcTarget.AllBuffered, lazadiocost, shoppicacost, nefixiacost, churchcost);
    }

    [PunRPC]
    void Loupcost(int lazadicos, int shoppicos, int nefixicos, int churchcos)
    {
    ...
    {
        if (player.GetComponent<TacticsMove>().guildConfirm == "Loup" && mainEvent.GetComponent<MainEventSys>().countTurn2 != 0)
        {
            if (citynumber == 1)
            {
    ...
    public void Spcialceti()
    {
        if (player != null)
        {
            if (player.GetComponent<PlayMoment>().guildConfirm == "Loup")
            {
                if (player.GetComponent<PlayMoment>().certificate[0] != 0)
                {
