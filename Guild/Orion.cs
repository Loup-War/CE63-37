...
public class Orion : MonoBehaviour
{
    ...
    public void LazadioRelation()
    {
        ...
        if (player.GetComponent<TacticsMove>().guildConfirm == "Orion")
        {
    ...
    public void SharingOrion()
    {
        pv.RPC("Orioncost", RpcTarget.AllBuffered, lazadiocost, shoppicacost, nefixiacost, churchcost);
    }

    [PunRPC]
    void Orioncost(int lazadicos, int shoppicos, int nefixicos, int churchcos)
    {
    ...
    {
        if (player.GetComponent<TacticsMove>().guildConfirm == "Orion" && mainEvent.GetComponent<MainEventSys>().countTurn2 != 0)
        {
            if (citynumber == 1)
            {
    ...
    public void Spcialceti()
    {
        if (player != null)
        {
            if (player.GetComponent<PlayMoment>().guildConfirm == "Orion")
            {
                if (player.GetComponent<PlayMoment>().certificate[0] != 0)
                {
