...
public class Thorus : MonoBehaviour
{
    ...
    public void Buying(int id)
    {
        ...
        if (thorus == true)
        {
            ...
            if (id == 11 && amount11 >= 1)//อาวูธพิเศษ
            {
                if (player.GetComponent<PlayerMoney>().money >= price && inv.weightall + 12 <= 300)
                {
    ...
    [PunRPC]
    void thorusSharevalue(int order1, int order2, int order3, int order4, int order5, int order6, int order7, int order8, int order9, int order10, int order11, int order12)
    {
        amount2 -= order1;
        amount3 -= order2;
        amount6 -= order3;
