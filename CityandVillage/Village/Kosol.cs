...
public class Kosol : MonoBehaviour
{
    PhotonView pv;
    ...
    void OnTriggerEnter(Collider col)
    {
        ...
        if (col.gameObject.tag == "Player1" && player1.GetComponent<PhotonView>().IsMine == true)
        {
            ...
            shopInvillage.GetComponent<VisitVillage>().kosol = true;
            kosol = true;
            openkosol();
        }
    ...
    void OnTriggerExit(Collider col)
    {
        if ((col.gameObject.tag == "Player1" && player1.GetComponent<PhotonView>().IsMine == true))
        {
            ...
            kosol = false;
            shopInvillage.GetComponent<VisitVillage>().kosol = false;
    ...
    void kosolSharing()
    {
        pv.RPC("kosolSharevalue", RpcTarget.AllBuffered, buyitem1, buyitem2, buyitem3, buyitem4, buyitem5, buyitem6, buyitem7, buyitem8, buyitem9, buyitem10, buyitem11, buyitem12);
    }

    [PunRPC]
    void kosolSharevalue(float order1, float order2, float order3, float order4, float order5, float order6, float order7, float order8, float order9, float order10, float order11, float order12)
    {
        amount2 -= order1;
        amount3 -= order2;
        amount6 -= order3;
