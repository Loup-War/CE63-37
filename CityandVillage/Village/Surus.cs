...
public class Surus : MonoBehaviour
{
    PhotonView pv;
    ...
    void Update()
    {
        ...
        if (surus == true)
        {
            valuemoney2.text = (inv.valuee * (50 - (50 * cetificatevalue))).ToString() + " G";
            valuemoney3.text = (inv.valuee * (300 - (300 * cetificatevalue))).ToString() + " G";
            valuemoney4.text = (inv.valuee * (260 - (260 * cetificatevalue))).ToString() + " G";
            valuemoney6.text = (inv.valuee * (500 - (5000 * cetificatevalue))).ToString() + " G";
            valuemoney12.text = (inv.valuee * (260 - (260 * cetificatevalue))).ToString() + " G";
            ...
            weit12.text = (inv.valuee * 12).ToString() + " Kg";
        }
    }
    void OnTriggerEnter(Collider col)
    {
        ...
        if (col.gameObject.tag == "Player1" && player1.GetComponent<PhotonView>().IsMine == true)
        {
            ...
            shopInvillage.GetComponent<VisitVillage>().surus = true;
            surus = true;
            opensurus();
        }
    ...
    void OnTriggerExit(Collider col)
    {
        if ((col.gameObject.tag == "Player1" && player1.GetComponent<PhotonView>().IsMine == true))
        {
            ...
            surus = false;
            shopInvillage.GetComponent<VisitVillage>().surus = false;
    ...
    void surusSharing()
    {
        pv.RPC("surusSharevalue", RpcTarget.AllBuffered, buyitem1, buyitem2, buyitem3, buyitem4, buyitem5, buyitem6, buyitem7, buyitem8, buyitem9, buyitem10, buyitem11, buyitem12);
    }

    [PunRPC]
    void surusSharevalue(float order1, float order2, float order3, float order4, float order5, float order6, float order7, float order8, float order9, float order10, float order11, float order12)
    {
        amount2 -= order1;
        amount3 -= order2;
        amount6 -= order3;
    ...
    public void opensurus()
    {
        if (surus == true)
        {
            txtamount2.text = amount3.ToString() + "/100";
            txtprice2.text = (50f - (50f * cetificatevalue)).ToString();
            txtamount3.text = amount6.ToString() + "/10";
            txtprice3.text = (300f - (300f * cetificatevalue)).ToString();
            txtamount4.text = amount5.ToString() + "/35";
            txtprice4.text = (260f - (260f * cetificatevalue)).ToString();
            txtamount6.text = amount7.ToString() + "/5";
            txtprice6.text = (500f - (500f * cetificatevalue)).ToString();
            txtamount12.text = amount12.ToString() + "/35";
            txtprice6.text = (260f - (260f * cetificatevalue)).ToString();
    ...
    public void UseCetificate()
    {
        if (surus == true)
        {
            if (player.GetComponent<PlayMoment>().certificate[0] == 3)
            {
