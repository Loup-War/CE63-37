...
public class Renes : MonoBehaviour
{
    PhotonView pv;
    ...
    void OnTriggerEnter(Collider col)
    {
        ...
        if (col.gameObject.tag == "Player1" && player1.GetComponent<PhotonView>().IsMine == true)
        {
            ...
            shopInvillage.GetComponent<VisitVillage>().renes = true;
            renes = true;
            openrenes();
        }
    ...
    void OnTriggerExit(Collider col)
    {
        if ((col.gameObject.tag == "Player1" && player1.GetComponent<PhotonView>().IsMine == true))
        {
            ...
            renes = false;
            shopInvillage.GetComponent<VisitVillage>().renes = false;
    ...
    void renesSharing()
    {
        pv.RPC("renesSharevalue", RpcTarget.AllBuffered, buyitem1, buyitem2, buyitem3, buyitem4, buyitem5, buyitem6, buyitem7, buyitem8, buyitem9, buyitem10, buyitem11, buyitem12);
    }

    [PunRPC]
    void renesSharevalue(float order1, float order2, float order3, float order4, float order5, float order6, float order7, float order8, float order9, float order10, float order11, float order12)
    {
        amount2 -= order1;
        amount3 -= order2;
        amount6 -= order3;
