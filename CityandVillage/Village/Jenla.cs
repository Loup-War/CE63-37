...
public class Jenla : MonoBehaviour
{
    PhotonView pv;
    ...
    void OnTriggerEnter(Collider col)
    {
        ...
        if (col.gameObject.tag == "Player1" && player1.GetComponent<PhotonView>().IsMine == true)
        {
            ...
            shopInvillage.GetComponent<VisitVillage>().jenla = true;
            jenla = true;
            openjenla();
        }
    ...
    void OnTriggerExit(Collider col)
    {
        if ((col.gameObject.tag == "Player1" && player1.GetComponent<PhotonView>().IsMine == true))
        {
            ...
            jenla = false;
            shopInvillage.GetComponent<VisitVillage>().jenla = false;
    ...
    void jenlaSharing()
    {
        pv.RPC("jenlaSharevalue", RpcTarget.AllBuffered, buyitem1, buyitem2, buyitem3, buyitem4, buyitem5, buyitem6, buyitem7, buyitem8, buyitem9, buyitem10, buyitem11, buyitem12);
    }

    [PunRPC]
    void jenlaSharevalue(float order1, float order2, float order3, float order4, float order5, float order6, float order7, float order8, float order9, float order10, float order11, float order12)
    {
        amount2 -= order1;
        amount3 -= order2;
        amount6 -= order3;
