...
public class Dijon : MonoBehaviour
{
    PhotonView pv;
    ...
    void OnTriggerEnter(Collider col)
    {
        ...
        if (col.gameObject.tag == "Player1" && player1.GetComponent<PhotonView>().IsMine == true)
        {
            ...
            shopInvillage.GetComponent<VisitVillage>().dijon = true;
            dijon = true;
            opendijon();
        }
    ...
    void OnTriggerExit(Collider col)
    {
        if ((col.gameObject.tag == "Player1" && player1.GetComponent<PhotonView>().IsMine == true))
        {
            ...
            dijon = false;
            shopInvillage.GetComponent<VisitVillage>().dijon = false;
    ...
    [PunRPC]
    void dijonSharevalue(float order1, float order2, float order3, float order4, float order5, float order6, float order7, float order8, float order9, float order10, float order11, float order12)
    {
        amount2 -= order1;
        amount3 -= order2;
        amount6 -= order3;
